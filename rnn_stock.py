...
for i in range(60, len(training_set_scaled)):   #从60到数据集结尾
    x_train.append(training_set_scaled[i - 60:i, 0])   #从0到60为训练集，61为标签；1-61为训练集，62天的为标签，依次下去
...
# 该应用只观测loss数值，不观测准确率，所以删去metrics选项，一会在每个epoch迭代显示时只显示loss值，故没给metrics赋值
